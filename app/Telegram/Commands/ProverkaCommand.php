...
class ProverkaCommand extends UserCommand
{

    /** @var string Command name */
    protected $name = 'proverka';
    /** @var string Command description */
    protected $description = '';
    /** @var string Usage description */
    protected $usage = '/proverka';
    /** @var string Version */
    ...
    public function execute(): ServerResponse
    {
        ...
        if ($state === 0 && $text && is_numeric($text)) {
            ...
            if ($task) {
                switch ($task->status) {
                    case 'active':
                        $msg = "Задача еще не завершена.";
                        $this->conversation->stop();
                        break;
                    case 'rejected':
                        $msg = "Задача уже отправлена на проверку.";
                        $this->conversation->stop();
                        break;
                    case 'finished': 
                        $msg = "Укажите причину отправки задачи на проверку:";
            ...
            foreach ($admins as $key => $admin) {
                $admin->notify(new MasterTaskRejected($task));
            }

            $msg = "Задача отправлена на проверку модератором. Ожидайте ответа...";
            ...
            return $this->replyToChat('Идентификатор задачи не обнаружен, после команды через пробел укажите номер задачи: /proverka 123');
        }
    }
}
